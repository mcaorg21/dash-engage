  ...
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE sistema IS NOT NULL AND sistema <> ''
        AND existe_sysemp_qives = false AND existe_qives_sysemp = false AND cancelada = false
      ORDER BY sistema
    `);
...
router.get('/lancamentos', async (req: AuthRequest, res) => {
  try {
    ...
    const filters = ['existe_sysemp_qives = false', 'existe_qives_sysemp = false', 'cancelada = false'];
    const values: string[] = [];
